...
def draw_monomial_plot(x, y_true, y_pred_dict, coefficient, degree, save_path):
    ...
    plt.title(f'Monomial: {coefficient:.2f}x^{degree}', fontsize=20, fontweight='bold')
...
def save_monomial_plots(x, y_true, y_pred_dict, coefficient, degree, base_path):
    # Create subfolders
    ...
    for model_name, y_pred in y_pred_dict.items():
        ...
        draw_monomial_plot(x, y_true, {model_name: y_pred}, coefficient, degree, save_path)
    
    # Save combined plot
    save_path = os.path.join(base_path, 'combined', 'combined_plot.pdf')
    draw_monomial_plot(x, y_true, y_pred_dict, coefficient, degree, save_path)